...
type Lead = {
  ...
  why_hot: string | null; became_hot_at: string | null; status?: 'New' | 'Drafted' | 'Sent';
};
...
export default function Home() {
  ...
  const loadLeads = useCallback(async () => {
    ...
    try {
      ...
      if (payload.leads[0]) setSelectedKey((current) => current || leadKey(payload.leads[0]));
  ...
  async function syncNow() {
    ...
    try {
      ...
      await loadLeads();
      setNotice('Hot leads are up to date.');
  ...
  return <main className="app-shell">
    <aside className="sidebar">
      ...
      <nav aria-label="Main navigation"><button className={view === 'leads' ? 'nav-item active' : 'nav-item'} onClick={() => setView('leads')} aria-label="Hot leads"><UserRound size={19} /></button><button className={view === 'settings' ? 'nav-item active' : 'nav-item'} onClick={() => setView('settings')} aria-label="Settings"><Settings2 size={19} /></button></nav>
      <div className="sidebar-spacer" /><div className="avatar">{account?.name?.[0] || 'Y'}</div>
    </aside>
    <section className="workspace">
      <header className="topbar"><div><div className="eyebrow"><span className="live-dot" /> EXPLEE · PROJECT 34448</div><h1>{view === 'leads' ? 'Hot leads' : 'Draft settings'}</h1></div><div className="top-actions"><div className="sync-copy"><strong>{apiBase ? 'Auto-sync on' : 'Preview mode'}</strong><span>{apiBase ? 'Every 10 minutes' : 'Connect backend to go live'}</span></div><Button variant="outline" className="quiet-button" onClick={() => void syncNow()} disabled={loading || !idToken}><RefreshCw size={15} className={loading ? 'spin' : ''} /> Sync now</Button><Button className="connect-button" onClick={() => void connectMicrosoft()}><span className="microsoft-mark"><i /><i /><i /><i /></span>{account ? account.username : 'Connect Microsoft 365'}</Button></div></header>
